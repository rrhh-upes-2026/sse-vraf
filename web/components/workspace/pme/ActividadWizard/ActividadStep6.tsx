...
export function ActividadStep6({ draft }: Props) {
  const missing: string[] = [];
  if (!draft.code?.trim())            missing.push("Código");
  if (!draft.name?.trim())            missing.push("Nombre");
  if (!draft.procesoId?.trim())       missing.push("Proceso");
  if (!draft.procedimientoId?.trim()) missing.push("Procedimiento");
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="divide-y divide-sse-border">
        ...
        <Row label="Duración"      value={draft.duracion ? `${draft.duracion}` : undefined} />
