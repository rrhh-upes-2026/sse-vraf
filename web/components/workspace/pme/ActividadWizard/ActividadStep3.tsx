...
export function ActividadStep3({ draft, patch }: Props) {
  const { data: tipos }          = usePMECatalogosPorTipo("tipoActividad");
      ...
      <div className="grid grid-cols-2 gap-4">
        <Field label="Tipo de Actividad">
          <Select
            ...
            onValueChange={(v) => patch({ tipoActividadId: v })}
            options={[
        ...
        <Field label="Estado Operativo">
          <Select
            value={draft.estadoOperativoId ?? ""}
            onValueChange={(v) => patch({ estadoOperativoId: v })}
            options={[
