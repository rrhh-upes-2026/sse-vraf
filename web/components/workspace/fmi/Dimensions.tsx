...
import {
  useFMIDimensions,
  useCreateFMIDimension,
  useUpdateFMIDimension,
  useDeleteFMIDimension,
} from "@/hooks/useFMI";
import type { FMIDimension, FMIStatus } from "@/types/fmi";
        ...
        <div className="space-y-3">
          <div className="grid grid-cols-2 gap-3">
            <div>
              ...
              <input value={form.codigo} onChange={set("codigo")} placeholder="DIM-01"
          ...
          <div>
            ...
            <input value={form.nombre} onChange={set("nombre")} placeholder="Eficiencia Operativa"
              className="w-full text-[12px] rounded border border-sse-border px-2 py-1.5 bg-sse-bg text-sse-ink focus:outline-none focus:ring-2 focus:ring-teal-500" />
          </div>
          <div>
...
export function FMIDimensions({ wsId }: { wsId: string }) {
  void wsId;
  const [creating, setCreating] = useState(false);
  const [editing, setEditing]   = useState<FMIDimension | null>(null);
  const [toDelete, setToDelete] = useState<FMIDimension | null>(null);
  const [search, setSearch]     = useState("");

  const { data: dimensions, isLoading } = useFMIDimensions();
  const create  = useCreateFMIDimension();
  const update  = useUpdateFMIDimension();
  const remove  = useDeleteFMIDimension();
  ...
  return (
    <div className="space-y-4">
      <div className="flex gap-2 items-center justify-between">
        <input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Buscar dimensión…"
      ...
      {isLoading ? (
        ...
        <div className="overflow-x-auto rounded-lg border border-sse-border">
          <table className="w-full text-[11px]">
            ...
            <tbody>
              {filtered.length === 0 && (
                <tr><td colSpan={6} className="text-center text-sse-muted py-8">Sin dimensiones registradas.</td></tr>
              )}
              {filtered.map((d) => (
                <tr key={d.id} className="border-b border-sse-border hover:bg-sse-surface/50">
                  <td className="px-3 py-2.5 text-sse-muted tabular-nums">{d.orden}</td>
                  <td className="px-3 py-2.5 font-mono text-teal-700">{d.codigo}</td>
                  <td className="px-3 py-2.5 font-medium text-sse-ink">{d.nombre}</td>
                  <td className="px-3 py-2.5 text-sse-muted hidden md:table-cell max-w-xs truncate">{d.descripcion || "—"}</td>
                  <td className="px-3 py-2.5 text-center">
                    <span className={`inline-block rounded-full px-2 py-0.5 text-[10px] font-medium ${d.estado === "activo" ? "bg-green-100 text-green-700" : "bg-sse-border text-sse-muted"}`}>
                      {d.estado}
                    </span>
                  </td>
                  <td className="px-3 py-2.5 text-right">
                    <div className="flex gap-1.5 justify-end">
                      <button onClick={() => setEditing(d)} className="text-[10px] px-2 py-1 rounded border border-sse-border text-sse-ink hover:bg-sse-border">Editar</button>
                      <button onClick={() => setToDelete(d)} className="text-[10px] px-2 py-1 rounded border border-red-200 text-red-600 hover:bg-red-50">Eliminar</button>
                    </div>
                  </td>
                </tr>
              ))}
      ...
      {creating && <DimensionModal initial={EMPTY} title="Nueva Dimensión" onSave={(f) => void handleCreate(f)} onClose={() => setCreating(false)} saving={create.isPending} />}
      {editing  && <DimensionModal initial={{ codigo: editing.codigo, nombre: editing.nombre, descripcion: editing.descripcion, orden: String(editing.orden), estado: editing.estado }} title="Editar Dimensión" onSave={(f) => void handleUpdate(f)} onClose={() => setEditing(null)} saving={update.isPending} />}

      {toDelete && (
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50">
          <div className="bg-sse-surface rounded-xl border border-sse-border p-6 w-80 space-y-4">
            <p className="text-[13px] font-semibold text-sse-ink">¿Eliminar dimensión?</p>
